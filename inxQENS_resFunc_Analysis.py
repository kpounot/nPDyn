...
#_Definitions of the slots for the plot window
    ...
    def plot3D(self):
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            meanWidth = np.mean([self.resFitList[k][j][0][1] 
                                for j, val in enumerate(fileDatas) 
                                    if 2 >= self.dataList[k][j].qVal >= 0.])
            stdWidth = np.std([self.resFitList[k][j][0][1] 
                                for j, val in enumerate(fileDatas) 
                                    if 2 >= self.dataList[k][j].qVal >= 0.])
    ...
    def analysisPlot(self):
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            for j, data in enumerate(fileDatas):    
                lauWList.append(self.resFitList[k][j][0][0])
                gauWList.append(self.resFitList[k][j][0][1])
                bkgdList.append(self.resFitList[k][j][0][2])
            ...
            ax1.set_ylabel(r'$Lorentzian Width $', fontsize=18)
            ax1.set_xlabel(r'$q$', fontsize=18)
            ax1.grid(True)
            ax1.legend(loc='upper center', bbox_to_anchor=(0.5, 1.4)).get_frame().set_alpha(0.5)
            ...
            ax2.errorbar(qList, gauWList, marker='o', 
                    label='...' + self.dataFiles[k][self.dataFiles[k].rfind('/'):])
            ax2.set_ylabel(r'$Gaussian Width $', fontsize=18)
            ...
            ax3.errorbar(qList, bkgdList, marker='o', 
                    label='...' + self.dataFiles[k][self.dataFiles[k].rfind('/'):])
            ax3.set_ylabel(r'$background $', fontsize=18)
