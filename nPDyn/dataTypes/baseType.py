...
class BaseType:
    ...
    def normalize(self, normFactorName='amplitude'):
        """ Normalizes data using a list of scaling factors from
            ...
            Parameters
            ----------
            normFactorName : str, optional
                name for the parameter giving the amplitude of the resolution
                function. This parameter is used to normalize the data.
                (default 'amplitude')

        """
        # check if `resData` is set and if it has been fitted
        if self.resData is None or self.resData._fit is None:
            raise AttributeError(
                "This dataset has no data for resolution function "
                "attached to it or these data were not fitted.")

        # assume a q-wise fit of normalized resolution function with a
        # parameter named 'amplitude'.
        normFList = np.array([val for key, val in self.resData._fit.best_values
                              if normFactorName in key])
    ...
    def fit(self, kwargs=None):
        ...
        ----------
        qwise : bool
            whether the fit is independent for each momentum transfer
            q value or not
        kwargs : dict
        ...
        """
        fit_kwargs = {
            'data': self.data.intensities,
            'weights': self.data.errors,
            'x': self.data.energies,
            'q': self.data.selQ}

        if kwargs is not None:
            fit_kwargs.update(kwargs)

        self._fit = self.model.fit(**fit_kwargs)
