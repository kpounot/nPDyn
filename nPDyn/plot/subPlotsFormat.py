...
def subplotsFormat(caller, sharex=False, sharey=False,
                   projection=None, params=False, FWS=False):
    ...
    if listSize == 1:
        caller.figure.subplots(1, listSize, sharex=sharex, sharey=sharey,
    ...
    if listSize == 4:
        caller.figure.subplots(2, 2, sharex=sharex, sharey=sharey,
                               subplot_kw={'projection': projection})
...
def subplotsFormatWithColorBar(caller, sharex=False, sharey=False,
    ...
    # Generating the subplots
    if listSize == 1:
        caller.figure.subplots(1, 2, sharex=sharex, sharey=sharey,
                               subplot_kw={'projection': projection},
                               gridspec_kw={'width_ratios': [20, 1]})

    if listSize != 1 and listSize < 4:
        caller.figure.subplots(1, 2 * listSize, sharex=sharex, sharey=sharey,
                               subplot_kw={'projection': projection},
                               gridspec_kw={'width_ratios':
                                            listSize * [20, 1]})

    if listSize == 4:
        caller.figure.subplots(2, 4, sharex=sharex, sharey=sharey,
                               subplot_kw={'projection': projection},
                               gridspec_kw={'width_ratios': 2 * [20, 1]})

    if listSize > 4 and listSize <= 9:
        nbrRows = int(np.ceil(listSize / 3))
        caller.figure.subplots(nbrRows, 6, sharex=sharex, sharey=sharey,
                               subplot_kw={'projection': projection},
                               gridspec_kw={'width_ratios': 3 * [20, 1]})

    if listSize > 9 and listSize <= 12:
        nbrRows = int(np.ceil(listSize / 4))
        caller.figure.subplots(nbrRows, 8, sharex=sharex, sharey=sharey,
                               subplot_kw={'projection': projection},
                               gridspec_kw={'width_ratios': 4 * [20, 1]})

    if listSize > 12:
        nbrRows = int(np.ceil(listSize / 5))
        caller.figure.subplots(nbrRows, 10, sharex=sharex, sharey=sharey,
                               subplot_kw={'projection': projection},
