...
class Window(QDialog):
    def __init__(self, dataFiles, resFitList, D2OParams):
        ...
        self.dataList = h5process.processData(dataFiles, karg['binS'], averageFiles=False, FWS=True)
        ...
        for fileIdx, dataSet in enumerate(self.dataList):
            for dEIdx, dEData in enumerate(dataSet):
                qIdxRange = [qIdx for qIdx, qVal in enumerate(dEData.qVals) 
                                        if qMin < qVal and qVal < qMax]
                self.dataList[fileIdx][dEIdx] = dEData._replace( 
                            qVals = dEData.qVals[qIdxRange[0]:qIdxRange[-1] + 1],
                            intensities = dEData.intensities[qIdxRange[0]:qIdxRange[-1] + 1],
                            errors = dEData.errors[qIdxRange[0]:qIdxRange[-1] + 1] )
...
#_Everything needed for the fit
    def fitFunc(self, params, dataSet, fileIdx, scanIdx, returnCost=True, qValIdx=0):
        ...
        for qIdx, qWiseData in enumerate(dataSet[0].intensities):
            ...
            dEData   = np.array([dEData.intensities[qIdx, scanIdx] for dEData in dataSet])
            dEErrors = np.array([dEData.errors[qIdx, scanIdx] for dEData in dataSet])
            ...
            aD2O        = self.D2OParams[0][qIdx].x[1]
            gD2O        = self.sD2O(temp, dataSet[0].qVals[qIdx])

            qVal = dataSet[0].qVals[qIdx]

            a0   = params[0] 
            beta = params[3]
            tau  = params[4]
            gGlo = params[1] * qVal**2 
            #gInt = params[2] 
            gInt = params[2] * qVal**2 / (1 + params[2] * qVal**2 * tau) 
            ...
            model   = ( beta * ( a0 * (S*voigt_G_g0 + (1-S)*voigt_G_g1)
                                      + (1-a0) * (S*voigt_I_g0 + (1-S)*voigt_I_g1)) 
                        + self.volFraction * aD2O * (S*voigt_D2O_g0 + (1-S)*voigt_D2O_g1)
                        + resParams[-1]) 


            modelList.append(model)
            cost += np.sum( (dEData / normF - model)**2  * (dEData / dEErrors)**2 )   


        if returnCost:
            return cost
        else:
            return modelList[qValIdx] 
    ...
    def basinHopping_fit(self):
        ...
        for i, dataSet in enumerate(self.dataList):
            ...
            bounds      = [(0., 1), (0., np.inf), (0., np.inf), (0., np.inf), (0., np.inf)] 
            params_init = [0.2, 15, 30, 0.1, 10]

            fitList = []
            for scanIdx, scanValues in enumerate(dataSet[0].intensities[0]): 
                ...
                fitList.append(optimize.basinhopping(self.fitFunc, 
                                ...
                                disp=True,
                                minimizer_kwargs={ 'args':(dataSet, i, scanIdx, True),
                                                   'bounds':bounds }))
...
#_Definitions of the slots for the plot window
    ...
    def QENS_3D(self):
        ...
        for k, dataSet in enumerate(self.dataList):
                ...
                for scanNbr, scanValue in enumerate(dataSet[0].intensities[0]):
                    ...
                    model = self.fitFunc(self.protFitList[k][scanNbr].x, dataSet, k,
                                         scanNbr, False, qValIdx)  
    ...
    def analysisPlot(self):
        ...
        for k, dataSet in enumerate(self.dataList):
            ...
            scanList = np.arange(len(self.protFitList[k]))
            a0List   = [val.x[0] for val in self.protFitList[k]]
            g0List   = [val.x[1] for val in self.protFitList[k]]
            g1List   = [val.x[2] for val in self.protFitList[k]]
            betaList = [val.x[3] for val in self.protFitList[k]]
    ...
    def fitPlot(self):
        ...
        for k, dataSet in enumerate(self.dataList):
            ...
            a0   = self.protFitList[k][scanNbr].x[0]
            g0   = self.protFitList[k][scanNbr].x[1]
            beta = self.protFitList[k][scanNbr].x[3]
            ax.plot(dEList, 
                    beta * a0 * g0**2 
                    / (g0**2 + dEList**2),
                    ls='--',
            ...
            a0 = self.protFitList[k][scanNbr].x[0]
            g1 = self.protFitList[k][scanNbr].x[2]
            beta = self.protFitList[k][scanNbr].x[3]
            ax.plot(dEList, 
            ...
            temp    = dataSet[0].temp
            gD2O    = self.sD2O(temp, dataSet[0].qVals[qValIdx]) 
            ax.plot(dEList, 
                    self.D2OParams[0][qValIdx].x[1] * self.volFraction 
                    * gD2O / ( gD2O**2 + dEList**2 ),
                    ls='-.',
                    zorder=5,
                    label="D2O solvent")
            ...
            #_Compute and plot model function
            model = self.fitFunc(self.protFitList[k][scanNbr].x, dataSet, k, scanNbr, False, qValIdx) 
