...
class Model(DataTypeDecorator):
    ...
    def getD2OSignal(self, qIdx=None):
        ...
        D2OSignal = np.array([
            self.model(self.params[idx].x, self, idx, False)
    ...
    def getParams(self, qIdx=None):
        """ Method used to obtain the fitted parameters for
            a given index of momentum transfer q.

            :arg qIdx: index momentum transfer q to be used.
                       If None, parameters for all q are returned.
...
def model(params, dataset, qIdx=None, returnCost=True):
    """ This class can be used to fit data from D2O - q-wise or globally -
        using a single lorentzian and D2O data from IN6 at the ILL.
