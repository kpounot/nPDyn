...
class Model(DataTypeDecorator):
    """ This class provides a model for protein dynamics in powder state
        using two Lorentzians.

        The model (:py:func:`~fitQENS_models.protein_powder_2Lorentzians`)
        is given by:
        ...
        .. math::
            ...
            S(q, \\omega) = e^{ -q^{2} \\langle u^{2} \\rangle / 3}
                            ...
                            + a_{2} \\mathcal{L}_{\\Gamma_{2} } \\right]
                            + bkgd
...
# -------------------------------------------------
    ...
    def getSubCurves(self, qIdx):
        ...
        resF, lor1, lor2 = self.model(self.getParams(qIdx),
                                      self, qIdx, False, True)
        labels = [r'$\mathcal{L}_{\Gamma_{1}}$',
                  r'$\mathcal{L}_{\Gamma_{2}}$']
