...
class Model(DataTypeDecorator):
    """ This class provides a model for protein dynamics in liquid state
        ...
        for global diffusion motions, :math:`\\mathcal{L}_{\\Gamma}` is a
        Lorentzian of width obeying jump diffusion model as decribed by
        Singwi and Sjölander [#]_ , :math:`\\mathcal{L}_{D_{2}O}` is the
    ...
    def __init__(self, dataType):
        ...
        self.model      = model
        self.params     = None
        self.paramsNames = ['$\Gamma$', '$\gamma$', '$\\tau$',
                            '$\\beta$', '$a_0$']
        self.BH_iter    = 20
        self.disp       = True
    ...
    def fit(self, p0=None, bounds=None):
        ...
        if bounds is None:  # Using default bounds
            bounds = [(0, self.data.X.max() * 3) for i in range(2)]
            bounds += [(0., np.inf)]
...
# -------------------------------------------------
    def getParams(self, qIdx):
        ...
        if len(self.params[0].x) == len(self.paramsNames):
            ...
            params[1] /= (1 + params[1] * qVal**2 * params[2])
    ...
    def getSubCurves(self, qIdx):
        ...
        resF, gLor, iLor = self.model(self.getParams(qIdx),
                                      self,
                                      D2OSignal,
                                      qIdx,
                                      False,
