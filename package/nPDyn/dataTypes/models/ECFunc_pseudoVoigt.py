...
from nPDyn.dataTypes.ECType import DataTypeDecorator



class Model(DataTypeDecorator):
    """ This class stores data as resolution function related.
        It allows to perform a fit using a pseudo-voigt profile as a model
        for instrument resolution given by:

        .. math::

           S(q, \\omega ) = A \\left[ S \\frac{ \\Gamma }{
                            \\pi ( (\\omega - shift)^{2} + \\Gamma^{2} ) }
                            + \\frac{ (1-S) }{ \\sqrt{2 \\pi } \\gamma }
                            e^{ -(\\omega - shift)^{2} / 2\\gamma^{2} }
                            \\right] + bkgd

        where *q* is the scattering angle, :math:`\\omega` the energy offset,
        A a scaling factor, S a scalar between 0 and 1, shift a scalar to
        account for maximum not being at 0, and bkgd a background term.
    ...
    def __init__(self, dataType):
        ...
        self.paramsNames = ["normF", "S", "lorW", "gauW", "shift", "bkgd"]


    def fit(self, p0=None, bounds=None):
        """ Uses Scipy's curve_fit routine to fit the pseudo-Voigt profile
            to the experimental data given in the argument resData.

        """

        # Fitted parameter are stored here for each q value in the dataSet
        ECList = []

        # Calling the scipy's curve_fit routine
        for qIdx, qWiseData in enumerate(self.data.intensities):
            ...
            if not bounds:
                bounds = ([0., 0., 0., 0., -10, 0.],
                          [np.inf, 1, maxWidth, maxWidth, 10, np.inf])
