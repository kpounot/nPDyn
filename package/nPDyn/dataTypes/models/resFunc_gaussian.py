...
class Model(DataTypeDecorator):
    """ This class provides a model to fit a resolution function using
        a sum of two normalized gaussians scaled by a common factor,
    ...
    def __init__(self, dataType):
        ...
        self.params     = None
        self.paramsNames = ["normF", "S", "g0",
                            "g1", "shift", "bkgd"]  # For plotting purpose
    ...
    def resFunc(self, x, normF, S, g0, g1, shift, bkgd):
        ...
        return (normF * (S * np.exp(-(x - shift)**2 / (2 * g0**2))
                / (g0 * np.sqrt(2 * np.pi))
                + (1 - S) * np.exp(-(x - shift)**2 / (2 * g1**2))
                / (g1 * np.sqrt(2 * np.pi)) + bkgd))




    def resFit(self, p0=None, bounds=None):
        """ Uses Scipy's curve_fit routine to fit the double gaussian
        ...
        for qIdx, qWiseData in enumerate(self.data.intensities):
            ...
            if not bounds:
                bounds = ([0., 0., 0., 0., -10, 0.],
                          [maxI, 1, maxWidth, maxWidth, 10, maxBkgd])
