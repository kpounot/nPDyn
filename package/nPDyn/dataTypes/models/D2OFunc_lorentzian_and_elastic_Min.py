...
class Model(DataTypeDecorator):
    """ This class provides a model for :math:`D_{2}O` signal fitting.
        ...
        .. math::
           ...
           S(q, \\omega ) = R(q, \\omega ) \\otimes \\left[ a_{0}
                            \\delta (\\omega )
                            + a_{1} \\frac{ \\Gamma_{D_{2}O}(q) }{ \\pi
                            ( \\omega^{2} + \\Gamma_{D_{2}O}^{2}(q) ) }
                            \\right] + bkgd

        where *q* is the scattering angle, :math:`\\omega` the energy
        offset, :math:`a_{0}` and :math:`a_{1}` are scalars,
        and *bkgd* a background term, and R the resolution function.
    ...
    def __init__(self, dataType):
        ...
        self.model       = model
        self.params      = None
        self.paramsNames = ["a0", "a1", "bkgd"]  # For plotting purpose
    ...
    def fit(self, p0=None, bounds=None):
        """ Only a q-wise fitting procedure is available here
            as it should be done for coherent :math:`D_{2}O` signal.

        """

        if self.disp:
            print("\nUsing Scipy's minimize to fit data from file: %s"
    ...
    def getD2OContributionFactor(self):
        """ Returns the contribution factor of
            :math:`D_{2}O` lineshape in the model """
    ...
    def getD2OSignal(self):
        """ Computes :math:`D_{2}O` line shape for each q values.
            ...
            If a qIdx is given, returns :math:`D_{2}O` signal
            only for the corresponding q value. """
    ...
    def getParams(self, qIdx=None):
        """ Method used to obtain the fitted parameters for a given
...
def model(params, dataset, qIdx=None, returnCost=True):
    """ This class can be used to fit data from
        :math:`D_{2}O` - q-wise or globally -
        using a single lorentzian and :math:`D_{2}O`
        data from IN6 at the ILL.
    ...
    a0      = params[0]  # contribution factor of elastic peak
    a1      = params[1]  # contribution factor of D2O
    bkgd    = params[2]  # background term
    ...
    model = model / (np.pi * (X**2 + gD2O**2))  # Computes the lorentzians
    ...
    cost = np.sum((dataset.data.intensities - model)**2
                  / dataset.data.errors**2, axis=1)
