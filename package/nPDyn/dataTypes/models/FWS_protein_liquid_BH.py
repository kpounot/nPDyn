...
class Model(DataTypeDecorator):
    """ This class provides a model for protein dynamics in liquid state
        ...
        :math:`a_{0}` acts as an EISF, and :math:`\\beta`
        and :math:`\\beta_{D_{2}O}` are scalars.
    ...
    def fit(self, p0=None, bounds=None):
        ...
        for tIdx in range(self.data.intensities.shape[0]):
            ...
            if p0 is None:  # Using default initial values
                p0  = [15, 30, 1]
                p0  = p0 + [np.max(self.data.intensities[tIdx, i])
            ...
            # each q-values (makes code for plotting easier)
...
# -------------------------------------------------
    ...
    def getParamsErrors(self, qIdx):
        """ Accessor for parameter errors of the
            model for the given q value
    ...
    def getWeights_and_lorWidths(self, qIdx):
        ...
        if len(self.params[0][0].x) == 5:
            for tIdx in range(self.data.intensities.shape[0]):
                weights = [self.params[tIdx][qIdx].x[4],
                           1 - self.params[tIdx][qIdx].x[4]]
