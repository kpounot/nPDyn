...
try:
    ...
    print('\nAbsorption correction libraries are not available. Paalman_Pings correction cannot be used.\n'
            + 'Verify that GSL libraries are available on this computer, and the path was correctly \n'
...
class BaseType:
    ...
    def setQRange(self, minQ, maxQ):
        ...
        ids = np.argwhere(np.logical_and(self.data.qVals > minQ, self.data.qVals < maxQ)).flatten()
