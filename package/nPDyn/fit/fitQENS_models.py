...
def protein_liquid_1Lorentzian(params, dataset, D2OSignal=None, qIdx=None, returnCost=True, 
                                                        scanIdx=slice(0,None), returnSubCurves=False):
    """ Fitting function for protein in liquid D2O environment.
    ...
    beta    = params[1]     #_contribution factor of protein, same shape as # of q-values used
    a0      = params[2]     #_contribution factor of EISF
    ...
    if isinstance(dataset.resData, resFunc_pseudoVoigt.Model):
        conv_resG0 = (g0 + resG0) / (np.pi * (X**2 + (g0 + resG0)**2))
        conv_resG1 = wofz((X + 1j*g0) / (resG1 * np.sqrt(2))).real / (resG1 * np.sqrt(2*np.pi))

    elif isinstance(dataset.resData, resFunc_gaussian.Model):
        conv_resG0 = wofz((X + 1j*g0) / (resG0 * np.sqrt(2))).real / (resG0 * np.sqrt(2*np.pi))
        conv_resG1 = wofz((X + 1j*g0) / (resG1 * np.sqrt(2))).real / (resG1 * np.sqrt(2*np.pi))
    ...
    model = ( a0 * resFunc + (1-a0) * ( resS*conv_resG0 + (1-resS)*conv_resG1) ) 
    ...
    if returnCost:
        ...
        res = a0 * resFunc
        Lor = beta * (1-a0) * ( resS*conv_G_resG0 + (1-resS)*conv_G_resG1 )
...
def protein_liquid_2Lorentzians(params, dataset, D2OSignal=None, qIdx=None, returnCost=True, 
                                                        scanIdx=slice(0,None), returnSubCurves=False):
    """ Fitting function for protein in liquid D2O environment.
