...
class ECPlot(QWidget):
    """ This class creates a PyQt widget containing a matplotlib
        canvas to draw the plots,a QLineEdit widget to allow the
    ...
    def plot(self):
        ...
        for idx, subplot in enumerate(ax):
            ...
            qValToShow = min(
                self.dataset[idx].data.qVals,
                key = lambda x: abs(float(self.lineEdit.text()) - x))
            qValIdx = int(np.argwhere(
                self.dataset[idx].data.qVals == qValToShow)[0])
    ...
    def analysisPlot(self):
        ...
        for fileIdx, dataset in enumerate(self.dataset):
            # Create 2D numpy array to easily access the q
            # dependance of each parameter
            paramsList = np.column_stack((params[0] for params
                                          in dataset.params))

            qList = dataset.data.qVals

            for idx, subplot in enumerate(ax):
                subplot.plot(qList, paramsList[idx], marker='o',
    ...
    def resPlot(self):
        ...
        for idx, subplot in enumerate(ax):
            ...
            subplot.plot(self.dataset[idx].data.X,
                         self.dataset[idx].model(self.dataset[idx].data.X,
