...
class Window(QDialog):
    def __init__(self):
        ...
        #↔_Parsing some additional parameters
        try:
    ...
    def scatFit(self):
        ...
        for i, fileData in enumerate(self.dataList):
            ...
            scatList1.append(optimize.basinhopping(self.fitFunc,
                    ...
                    disp=True,
                    minimizer_kwargs={'args': (fileData, i),
                                      'bounds': bounds}))
...
#_Definitions of the slots for the plot window
    ...
    def analysisPlot(self):
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            g0Err.append(np.sqrt(
                            np.diag(self.scatFitList1[k].lowest_optimization_result.hess_inv.todense()))[0])
            g1List.append(self.scatFitList1[k].x[1])
            g1Err.append(np.sqrt(
                            np.diag(self.scatFitList1[k].lowest_optimization_result.hess_inv.todense()))[1])
            s0List.append(self.scatFitList1[k].x[2])
            s0Err.append(np.sqrt(
                            np.diag(self.scatFitList1[k].lowest_optimization_result.hess_inv.todense()))[2])
            s1List.append(self.scatFitList1[k].x[3])
            s1Err.append(np.sqrt(
                            np.diag(self.scatFitList1[k].lowest_optimization_result.hess_inv.todense()))[3])
            s2List.append(self.scatFitList1[k].x[4])
            s2Err.append(np.sqrt(
                np.diag(self.scatFitList1[k].lowest_optimization_result.hess_inv.todense()))[4])
