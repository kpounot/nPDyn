...
def inxPlot(dataFile, binS, qMin, qMax):
    ...
    ratioPower = float(kargList['ratioPower'])
    ...
    for i, val in enumerate(dataList[0].temps):
        ...
        normSList.append([(q.intensities[i] - resFitList[0][j][0][3]) 
                        #/ (resFitList[0][j][0][3] - resFitList[0][j][0][5])
                        / np.mean(np.array(q.intensities[:normFact]) - resFitList[0][j][0][3])
...
if __name__ == '__main__':
    ...
    resFitList, meanWidth = resAnalysis.getDataList()
