...
resFunc, resCost, resGrad = tf.resFunc_pseudo_voigt()
...
class Window(QDialog):
    def __init__(self):
        ...
        #_Additional input parameters verifications
        try:
...
#_Everything needed for the fit
    def res_cost(self, x, data):

        cost = resCost(x, data.energies, data.intensities, data.errors+0.0001).astype('float64') 
        grad = resGrad(x, data.energies, data.intensities, data.errors+0.0001).astype('float64')
    ...
    def resFit(self):
        ...
        bounds_min = [(0., 2000), (0., 1), (0., 1), (0., 1), (-5, 5), (0., 0.5)]

        for i, resFile in enumerate(self.dataList):
            resList = []
            for j, data in enumerate(resFile):
                resList.append(optimize.basinhopping(self.res_cost,
                                                 x0,
                                                 disp=True,
                                                 niter=self.BH_iter,
                                                 ...
                                                 interval=10,
                                                 T=500,
                                                 minimizer_kwargs={'args': (data,), 'jac': True,
                                                                   'bounds': bounds_min}))
                ...
                print('Fit bkgd  : ' + str(resList[j].x[5]) + '\n', flush=True)
            ...
            for j, qDatas in enumerate(fileDatas):
                if qDatas.qVal == qValToShow:
                    ax.errorbar(qDatas.energies, 
                           [x/self.resFitList[k][j].x[0] for x in qDatas.intensities],
                           [x/self.resFitList[k][j].x[0] for x in qDatas.errors], fmt='o')
    ...
    def plot3D(self):
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            for i, datas in enumerate(fileDatas):
                normFact = self.resFitList[k][i].x[0]
    ...
    def analysisPlot(self):
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            for j, data in enumerate(fileDatas):    
                lauWList.append(self.resFitList[k][j].x[2])
                gauWList.append(self.resFitList[k][j].x[3])
                bkgdList.append(self.resFitList[k][j].x[5])
        ...
        for k, fileDatas in enumerate(self.dataList):
            for j, qDatas in enumerate(fileDatas):
                if qDatas.qVal == qValToShow:
                    ...
                    normF = self.resFitList[k][j].x[0]
                    result = resFunc(self.resFitList[k][j].x, qDatas.energies)
