...
def inxPlot(dataFile, binS, qMin, qMax):
    ...
    for j, val in enumerate(dataList):
        plt.errorbar(dataList[0].temps, 
                     ...
                     fmt='-o', label='%.2f' % qList[j], c=cmap(normColors(val.qVal)))   
    ...
    def gammaModel(x, b, msd): 

        #return np.exp(-shift*(x*msd)**2) / (1 + ((x*msd)**2/b))**b  #_Shifted Gamma distribution
        return 1 / (1 + ((x*msd)**2/b))**b  #_Standard Gamma distribution
    ...
    for i, val in enumerate(dataList[0].temps):
        ...
        gammaFit = [gammaModel(qVal, fitGaussList[i][0][0], fitGaussList[i][0][1])
                       for qVal in qList]
...
def inxPlotMSD(dataFiles):
    ...
    for i, val in enumerate(fitList):
        ...
        fitYdata = [val[j][0][1]**2 for j, value in enumerate(dataList[i][0].temps) 
        ...
        ax.errorbar(dataList[i][0].temps,
                    ...
                    label=dataFiles[i][dataFiles[i].rfind('/'):], fmt='-o')
