...
#_Everything needed for the fit
    ...
    def fitFunc(self, x, datas, i, j):
        ...
        funSquared = sum([((GauR / self.meanResGauWidth[i] 
        ...
        for i, fileDatas in enumerate(self.dataList):
            ...
            for j, datas in enumerate(fileDatas):
                scatList.append(optimize.minimize(lambda x:
                    ...
                    [1, 4, 30, 0.1, 0.2, 0.2, 0.5, 1], 
                    bounds = [  (0.6, 30), (0.6, 30), (0., 1000), 
                ...
                print('Fit gb : ' + str(scatList[j].x[2]), flush=True)
                print('Fit s0 : ' + str(scatList[j].x[3]), flush=True)
                print('Fit st : ' + str(scatList[j].x[4]), flush=True)
                print('Fit sr : ' + str(scatList[j].x[5]), flush=True)
                print('Fit sb : ' + str(scatList[j].x[6]), flush=True)
                print('Fit msd : ' + str(scatList[j].x[7]), flush=True)
    ...
    def analysisPlot(self):
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            for j, data in enumerate(fileDatas):    
                ...
                gbList.append(self.scatFitList1[k][j].x[2])
                gbErr.append(np.sqrt(np.diag(self.scatFitList1[k][j].hess_inv.todense()))[2])
            ...
            #_Plot of the gb parameter of the fits
            ax3 = self.figure.add_subplot(mplGrid[:,2])
            ax3.plot(qList, gbList, marker='o', 
                    label='...' + self.dataFiles[k][self.dataFiles[k].rfind('/'):])
            ax3.set_xlabel(r'$Background $', fontsize=14)
            ax3.grid(True)
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            for j, qDatas in enumerate(fileDatas):
                if qDatas.qVal == qValToShow:
                    ...
                    gb = self.scatFitList1[k][j].x[2]
                    s0 = self.scatFitList1[k][j].x[3]
                    st = self.scatFitList1[k][j].x[4]
                    sr = self.scatFitList1[k][j].x[5]
                    sb = self.scatFitList1[k][j].x[6]
                    msd = self.scatFitList1[k][j].x[7]
