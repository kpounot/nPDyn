...
#_Definitions of the slots for the plot window
    ...
    def analysisPlot(self):
        ...
        for k, fileDatas in enumerate(self.dataList):
            ...
            gammaErr.append(np.sqrt(np.diag(self.scatFitList1[k].hess_inv.todense()))[0])
            p1List.append(self.scatFitList1[k].x[1])
            p1Err.append(np.sqrt(np.diag(self.scatFitList1[k].hess_inv.todense()))[1])
            distList.append(self.scatFitList1[k].x[2])
            distErr.append(np.sqrt(np.diag(self.scatFitList1[k].hess_inv.todense()))[2])
            msdList.append(self.scatFitList1[k].x[3])
            msdErr.append(np.sqrt(np.diag(self.scatFitList1[k].hess_inv.todense()))[3])
