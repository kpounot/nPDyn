...
def vanaNormalization(datas):
    
    vanaMessage = QMessageBox.information(QWidget(), 'Vanadium file selection',
                        'Please select the file containing the vanadium measurements')
    vanaFile = QFileDialog().getOpenFileName()[0] 
    vanaDatas = inxBin(vanaFile, karg['binS'])

    for i, val in enumerate(datas):
        val = val._replace(intensities = [valI / vanaDatas[i].intensities[j] for j, valI 
                                in enumerate(val.intensities)])
        val = val._replace(errors = [valI / vanaDatas[i].intensities[j] for j, valI 
                                in enumerate(val.errors)])
...
if __name__ == '__main__':
    ...
    dataList = vanaNormalization(dataList)
