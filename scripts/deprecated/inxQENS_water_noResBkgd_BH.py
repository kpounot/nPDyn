...
#_Everything needed for the fit
    ...
    def scatFit(self):
        ...
        for i, filedata in enumerate(self.dataList):
            ...
            scatList1.append(optimize.basinhopping(lambda x:
                    ...
                    [1, 4, 0.1, 0.5, 0.4, 1, 2] + [0.002 for val in range(len(filedata))], 
                    minimizer_kwargs={'bounds':[(0.0, 30), (0.0, 30), 
                              (0., 1), (0., 1), (0., 1), (0., 10), (0., 3.)]
                               + [(0., 0.5) for val in range(len(filedata))]},
                    disp=True,
                    niter=50, interval=5, stepsize=1,
    ...
    def fittedFunc(self, data, i, resShift, normFact, lorS, GauR, lorR, sigma, 
        ...
        Model = [np.exp(-data.qVal**2 * msd**2) *
                ((s0 + sr * bessel[0][0]**2)   
                * resAnalysis.resFunc(val, lorR, GauR, lorS, 1, resShift, bkgd) + 
...
#_Definitions of the slots for the plot window
    ...
    def analysisPlot(self):
        ...
        for k, filedata in enumerate(self.dataList):
            ...
            ax.errorbar(xList2, gtList, gtErr, marker='o', 
            ...
            ax.errorbar(xList2, stList, stErr, marker='o', 
                    linestyle='-', label='st')
            ax.errorbar(xList, srList, srErr, marker='^', 
                    linestyle='-', label='sr')
            ax.errorbar(xList, s0List, s0Err, marker='x', 
                    linestyle='-', label='sr')
