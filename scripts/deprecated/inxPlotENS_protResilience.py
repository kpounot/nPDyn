'''
This script use a standard gaussian model to fit the data from
...
def inxPlotMSD(dataFiles):
    ...
    plt.figure()
    normColors = matplotlib.colors.Normalize(vmin=0, vmax=len(fitList))
    ...
    for i, val in enumerate(fitList):
        ...
        plt.errorbar(dataList[i][0].temps, [value[0][1] for value in val],
                    ...
                    label=dataFiles[i][dataFiles[i].rfind('/'):], c=cmap(normColors(i)))
        #plt.scatter(dataList[i][0].temps, [value[0][1] for value in val], s=50,
        #            label=labelList[i], c=colorList[i])
        

        #_Plot of the fitted spring constant
        fitYList = [fitFunc(x, popt[0], popt[1]) for x in dataList[i][0].temps]
        plt.plot(dataList[i][0].temps, fitYList, lw=4, ls='--', c=cmap(normColors(i)),
