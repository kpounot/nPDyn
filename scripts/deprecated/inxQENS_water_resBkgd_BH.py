...
#_Everything needed for the fit
    ...
    def scatFit(self):
        ...
        for i, filedata in enumerate(self.dataList):
            ...
            scatList1.append(optimize.basinhopping(lambda x:
                    ...
                    [4, 1, 0.1, 0.5, 0.4, 1, 0.002, 2], 
                    minimizer_kwargs={'bounds':[(0.0, 30), (0.0, 30), 
                              (0., 1), (0., 1), (0., 1), (0., 10), (0., 0.5), (0., 3.)]},
                    callback=self.fitState,
                    niter=50, interval=5, stepsize=0.5, niter_success=5,
            ...
            print('Fit pBkgd : ' + str(scatList1[i].x[6]), flush=True)
            print('Fit Lambda : ' + str(scatList1[i].x[7]), flush=True)
    ...
    def fitState(self, x, f, accepted):
        ...
        print('        {0:<50}= {1:.4f}'.format('pBkgd', x[6]), flush=True)
        print('        {0:<50}= {1:.4f}'.format('Lambda', x[7]), flush=True)
...
#_Definitions of the slots for the plot window
            ...
            for j, qdata in enumerate(filedata):
                if qdata.qVal == qValToShow:
                    ...
                    ax.errorbar(qdata.energies, 
                           [(GauR/self.meanResGauWidth[k])**self.ratioPower * x / normFact 
                                    for x in qdata.intensities],
        ...
        for k, filedata in enumerate(self.dataList):
            ...
            for i, data in enumerate(filedata):
                ...
                ax.plot(data.energies, 
                                  [(GauR/self.meanResGauWidth[k])**self.ratioPower * val/normFact 
    ...
    def analysisPlot(self):
        ...
        xList2 = [x+0.8 for x in range(len(self.dataFiles))]
        ...
        ax.plot(xList2, stList, marker='o', 
                linestyle='-', label='st')
        ax.plot(xList, srList, marker='^', 
                linestyle='-', label='sr')
        ax.plot(xList, s0List, marker='^', 
                linestyle='-', label='s0')
        ...
        for k, filedata in enumerate(self.dataList):
            ...
            for j, qdata in enumerate(filedata):
                if qdata.qVal == qValToShow:
                    ...
                    pBkgd = self.scatFitList1[k].x[6]
                    diffFactor = self.scatFitList1[k].x[7]
