...
def inxPlotMSD(dataFiles):
    ...
    for idx, datafile in enumerate(dataFiles):
        ...
        summedIntensities = [0 for i in dataList[idx][0].intensities]
    ...
    for i, val in enumerate(fitList):
        ...
        #reducedTList = dataList[i][0].temps[:np.where(dataList[i][0].temps > 298)[0][0]]
        plt.errorbar(dataList[i][0].temps, 
