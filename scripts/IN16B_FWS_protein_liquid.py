...
class Window(QDialog):
    def __init__(self, dataFiles, resFitList, D2OFitList):
        super().__init__()
...
#_Everything needed for the fit
    def modelFunc(self, x, D2OPeakSignal, qVal, temp, a0, g0, g1, beta):
        ...
        return ( beta * (a0 * g0 / (g0**2 + x**2) + (1-a0) * (g0 + g1) / ( (g0 + g1)**2 + x**2 )) 
    ...
    def fitFunc(self, params, dataSet, fileIdx, qIdx, dEList, dEData, dEErrors, returnCost=True):
        ...
        model   = self.modelFunc(dEList, D2OPeakSignal, dataSet[0].qVals[qIdx], temp, *params)
        resFunc = self.resFunc(dEList, 1, *resParams[1:])
    ...
    def basinHopping_fit(self):
        ...
        for i, dataSet in enumerate(self.dataList):
            ...
            bounds  = [(0., 1), (0.7, 100), (0.7, 100), (0., 1)] 
            ...
            for qIdx, qWiseData in enumerate(dataSet[0].intensities):
                ...
                for scanIdx, scanValues in enumerate(qWiseData): 
                    ...
                    if scanIdx == 0:
                        params_init = [0.5, 5, 20, 1]
                    else:
                        params_init = fitList[scanIdx-1].x
...
#_Definitions of the slots for the plot window
    ...
    def fitPlot(self):
        ...
        for k, dataSet in enumerate(self.dataList):
            ...
            ax.plot(dEList, 
                    D2OPeakSignal * self.volFraction * gD2O / ( gD2O**2 + dEList**2 ),
                    ls='-.',
                    zorder=5,
                    label="D2O solvent")
