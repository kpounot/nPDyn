...
def inxPlot(dataFile, binS, qMin, qMax):
    ...
    def fCorrGauss(x, a, b, c):
        return a * (np.exp(-(1/6)*(x**2)*b) * (1 + (1/72)*(x**4)*c))
    ...
    for i, val in enumerate(dataList[0].temps):
        ...
        gaussianFit = [fCorrGauss(qVal, fitGaussList[i][0][0], fitGaussList[i][0][1],
                       fitGaussList[i][0][2]) for qVal in qList]
...
def inxPlotMSD(dataFiles):
    ...
    for i, val in enumerate(fitList):
        ...
        ax.errorbar(dataList[i][0].temps, [value[0][1] for  value in val],
                    [np.sqrt(np.diag(value[1]))[1] for value in val], fmt='-o',
                    label='...' + dataFiles[i][len(datafile)-40:])
